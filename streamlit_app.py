...
def get_fruit_load_list():
  with my_cnx.cursor() as my_cur:
    ...
    return my_cur.fetchall() #Previous loading only one row: my_data_row = my_cur.fetchone()

#Add a button to load the fruit
if streamlit.button('Get Fruit Load List'):
  my_cnx = snowflake.connector.connect(**streamlit.secrets["snowflake"])
  my_data_rows = get_fruit_load_list() #Previous loading only one row: my_data_row = my_cur.fetchone()
...
if streamlit.button('Add a Fruit to the List'):
  my_cnx = snowflake.connector.connect(**streamlit.secrets["snowflake"])
  back_from_function = insert_ros_snowflake(add_my_fruit)
